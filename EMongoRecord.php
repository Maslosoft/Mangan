...
abstract class EMongoRecord extends EMongoEmbdedDocument
{
	...
	public function insert()
	{
		...
		if($this->beforeSave())
		{
			...
			if(empty($this->_id))
				unset($rawData['_id']);
			$this->getCollection()->insert($rawData, array('fsync'=>self::$_db->fsyncFlag));
	...
	public function update()
	{
		...
		if($this->beforeSave())
		{
			$this->getCollection()->save($this->toArray(), array('fsync'=>self::$_db->fsyncFlag));
			$this->afterSave();
		}
		else
			return false;
	...
	public function delete()
	{
		if(!$this->getIsNewRecord())
		{
			...
			{
				$result = $this->getCollection()->remove(array('_id'=>$this->_id), array('fsync'=>self::$_db->fsyncFlag, 'justOne'=>true));
				$this->afterDelete();
				$this->_id=null;
				$this->setIsNewRecord(true);
				return $result;
